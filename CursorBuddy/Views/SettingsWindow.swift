...
final class SettingsWindowController: NSObject, @unchecked Sendable {
    ...
    private override init() {
        let win = SettingsKeyWindow(
            contentRect: NSRect(x: 0, y: 0, width: 640, height: 520),
            styleMask: [.titled, .closable, .miniaturizable, .resizable],
            backing: .buffered,
            defer: false
        )
        win.title = "CursorBuddy Settings"
        win.center()
...
// MARK: - Settings Key Window

/// NSWindow subclass that always accepts key status so text fields work.
private class SettingsKeyWindow: NSWindow {
...
struct TTSSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("CursorBuddy speaks its responses aloud using these settings.")
                .font(.system(size: 12))
                .foregroundColor(.white.opacity(0.5))
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                shortcutRow(label: "Toggle CursorBuddy Panel", key: "⌘ + Shift + P")
                ...
                HStack(spacing: 6) {
                    ForEach(CursorStyle.allCases) { style in
                        Button {
                            ...
                            .glassEffect(
                                cursorConfig.style == style
                                    ? .regular.tint(.blue.opacity(0.25))
                                    : .regular.tint(.white.opacity(0.03)),
                                in: .rect(cornerRadius: 8)
                            )
                            .overlay {
                                RoundedRectangle(cornerRadius: 8)
                                    .stroke(cursorConfig.style == style ? Color.blue.opacity(0.5) : .white.opacity(0.08), lineWidth: 1)
                            }
                        }
                        .buttonStyle(.plain)
                    }
                ...
                HStack(spacing: 4) {
                    ForEach(CursorAppearanceConfiguration.menuBarIconOptions, id: \.symbol) { option in
                        Button {
                            ...
                            .overlay {
                                RoundedRectangle(cornerRadius: 8)
                                    .stroke(cursorConfig.menuBarIcon == option.symbol ? Color.blue.opacity(0.5) : .white.opacity(0.08), lineWidth: 1)
                            }
                        }
                        .buttonStyle(.plain)
                    }
...
struct PermissionsSettingsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Text("CursorBuddy requires these macOS permissions to function. Grant them in System Settings if needed.")
                .font(.system(size: 12))
                .foregroundColor(.white.opacity(0.5))
                .fixedSize(horizontal: false, vertical: true)
